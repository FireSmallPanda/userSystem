 ...
 *
 */
public class ProjectsArexcelUtil {
	public ProjectsArexcelUtil(String pathStr) {
	...
	private String tableTitle = "项目信息";// 表格标题
	private String tableLab = "projectstable";// 表格选项卡名
	private String[] tableClosName = { "项目ID", "项目名字", "开始时间", "结束时间", "状 态" };// 列名(该列名必须与实体类参数相对应)
	...
	public String genarateExcel(@SuppressWarnings("rawtypes") List list) throws Exception {
		...
		try {
			...
			startRowNum++;
			startColNum = 0;
			DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");// 转换date
			// 添加记录(此处需要手动添加)
			while (it.hasNext()) {
				Projects ar = (Projects) it.next();// 集合类型
				ws.addCell(new Label(startColNum, startRowNum, String.valueOf(ar.getProjectsId()), lwh_value));
				startColNum++;
				ws.addCell(new Label(startColNum, startRowNum, ar.getProjectsName(), lwh_value));
				startColNum++;
				ws.addCell(new Label(startColNum, startRowNum, dateFormat.format(ar.getProjectsStart()), lwh_value));
				startColNum++;
				ws.addCell(new Label(startColNum, startRowNum, dateFormat.format(ar.getProjectsEnd()), lwh_value));
				startColNum++;
				ws.addCell(new Label(startColNum, startRowNum,
						String.valueOf(dictUtil.getDictByKey("100001", "100", ar.getProjectsStatus().toString()).getDictContentCN()),
						lwh_value));
				startColNum++;
				// 将行数加1，列数重置为0
				startRowNum++;
